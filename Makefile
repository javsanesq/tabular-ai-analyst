...
.PHONY: install test api db-upgrade db-revision docker-up docker-down docker-config benchmark ui-build ui-e2e lint smoke-e2e smoke-openai
...
benchmark:
	PYTHONPATH=api/src APP_ENV=test DATABASE_URL=sqlite+pysqlite:///:memory: DATA_DIR=data/benchmark $(PYTHON) scripts/run_benchmark.py
...
smoke-openai:
	PYTHONPATH=api/src DATA_DIR=data/openai-smoke $(PYTHON) scripts/smoke_openai_planner.py
